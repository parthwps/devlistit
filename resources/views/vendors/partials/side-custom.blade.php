 ...
 <aside class="widget-area mb-40">
    <div class="widget radius-md">
      <ul class="links">
        ...
        @if ($support_status->support_ticket_status == 'active')
         ...
         <!--  <li
          ...
          <li class="{{ request()->routeIs('vendor.support_ticket') ? 'active' : '' }}">
                  <a href="{{ route('vendor.support_tickets') }}">
                    <span class="sub-item">{{ __('Messages') }}</span> @if(contactNotification(Auth::guard('vendor')->user()->id) > 0) <span style="background-color:#dc3545 !important; color:#fff;" class="badge bg-danger rounded-pill">{{contactNotification(Auth::guard('vendor')->user()->id)}}</span>@endif
                  </a>
        ...
        <li class="nav-item @if (request()->routeIs('vendor.payment_log')) active @endif">
          <a href="{{ route('vendor.recently.viewed') }}">
