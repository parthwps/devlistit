...
<section class="px-5 px-sm-5 px-lg-5 bg-red pt-20 pb-20 pt-sm-40 pb-sm-40 mt-sm-4  ">
    ...
    <div class="d-flex flex-sm-row flex-column justify-content-between align-items-center mb-3 px-1 px-sm-4">
        ...
        <div class="car-container" id="car-container">
            ...
            @foreach ($browse_by_lifestyle as $key => $value)
            @php
             $url = url_params[$key] ?? '#';
            @endphp
        <a href="{{ $url }}" class="browser_by_lifestyle"
         data-cat="{{ $value->custom_cat}}">
            <div class="car-box">
                <div class="d-flex justify-content-center align-items-center" style="height: 101px; width: 100%;">
                    <img width="180px" class="" 
                    src="{{'assets/img/'. $value->image_path}}" 
                         alt="First Car"></img>
                </div>
                <div class="text-center pt-10 pb-1" style="font-size: 16px; font-weight: 600;">{{$value->title}}</div>
                <div class="text-center carsNumber" style="font-size: 14px; font-weight: 400;" data-carsNumbers="{{$value->car_count}}">{{$value->car_count}}</div>
            </div>
        </a>
        @endforeach
    </div>
