    ...
    public function addCurrPackage(Request $request)
{
    try 
    {
        DB::transaction(function () use ($request) 
        {
            ...
            if ($vendor->amount < $selectedPackage->price) 
            {
                Session::flash('warning', 'Dealer account balance is less.!');
                return redirect()->back();
            }

            $total_blance_aval = $vendor->amount - $selectedPackage->price;

            Vendor::where('id', $vendor_id)->update(['amount' => $total_blance_aval]);

            Deposit::create([
                'amount' => $selectedPackage->price,
                'deposit_type' => 'withdrawl',
                'vendor_id' => $vendor_id,
                'short_des' => 'Balance deduct for buying package ' . $selectedPackage->title
            ]);
...
public function changeCurrPackage(Request $request)
{
    try {
        DB::transaction(function () use ($request) {
            ...
            $total_balance_available = $vendor->amount - $selectedPackage->price;

            Vendor::where('id', $vendor_id)->update(['amount' => $total_balance_available]);
    ...
    try {
        DB::transaction(function () use ($request) {
            ...
            Deposit::create([
                ...
                'deposit_type' => 'withdrawl',
                'vendor_id' => $vendor_id,
                'short_des' => 'Balance deducted for buying package ' . $selectedPackage->title
            ]);
