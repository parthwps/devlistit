...
{   
 ...
 // ------ Get all Make  ------
    ...
    // ---- get fuel Type -----
      ...
      {
        $CarConditionData = CarCondition::select('id', 'name')->where([['status', 1]])->orderBy('serial_number', 'asc')->get();
        if ($CarConditionData) {
         
          return response()->json(['code' => 200, 'message' => 'successful.','data' =>$CarConditionData]); exit;
          //return $models;
        } else {
          return response()->json(['code' => 200, 'message' => 'successful.','data' =>null]); exit;
        }
     ...
     foreach($data['Response']['DataItems']['RoiVehicleDetails'] as $key=>$val){
        if($key =="Transmission"){
          ...
          $apiarray[$key] =  $tData;
            ...
            $CarConditionData = CarCondition::select('id', 'name')->where([['name', $val], ['status', 1]])->orderBy('serial_number', 'asc')->first();
            $apiarray[$key] =  $CarConditionData;
        }  
