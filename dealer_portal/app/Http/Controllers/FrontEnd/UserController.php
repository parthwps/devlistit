...
class UserController extends Controller
{
  public function __construct()
  {
    ...
    function saveToDraft(Request $request)
    {
       ...
       if ($column_name == 'enquirey_person') {
    if ($storeAdInDraft) {
        // Decode the existing value from the database (ensure it's an array)
        $enquirey_person = !empty($storeAdInDraft->enquirey_person) ? json_decode($storeAdInDraft->enquirey_person, true) : [];

        // Ensure that after json_decode, the result is an array
        if (!is_array($enquirey_person)) {
            $enquirey_person = [];
        }
        ...
        // Re-encode the array or set to null if the array is empty
        $storeAdInDraft->enquirey_person = !empty($enquirey_person) ? json_encode(array_values($enquirey_person)) : null;
  ...
  public function authenticationViaProvider($driver)
  {
    ...
    $vendor->status = 1;
    $vendor->save();
      $in = ['vendor_id' => $vendor->id , 'language_id' => 20 , 'name' => $user->name ];
