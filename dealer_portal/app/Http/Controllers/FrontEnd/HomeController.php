...
class HomeController extends Controller
{
   ...
   function getEngineCapacity(Request $request)
    {
        ...
        if($request->subcatVal == 48 || $request->subcatVal == 62)
        {
            ...
            <input type="number" class="form-control" id="addCapacity" name="engineCapacity" onfocusout="saveDraftData(this, \'engine\')"  />
            </div>';   
        }
        else
        {
  ...
  public function defaultsuggestions(Request $request){
    ...
    public function vehicleDatatwo(Request $request){
    ...
    if($request->vehiclereg !=7007){    
      // Init cURL session
       ...
       // echo "cURL Error: " . $error; exit;
        ...
        if ($error) {
          ...
          // echo $apiarray["response"];
        // echo $data['Response']['StatusCode'];
        
          $apiarray["response"] = $data['Response']['StatusCode'] ;
          if($data['Response']['StatusCode']!="ItemNotFound" and $data['Response']['StatusCode']!="KeyInvalid") {
     ...
     // $apiarray["catfilters"] = json_decode($categories->filters);
      $filters = view('vendors.car.vehicledetails', compact('apiarray','categories'))->render();
      
      
      //echo json_encode($apiarray); exit;
    return response()->json(['code' => 200, 'message' => 'successful.','data' =>$filters]); exit;
