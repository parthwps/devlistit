...
class VendorCheckoutController extends Controller
{
    ...
    public function store($request, $transaction_id, $transaction_details, $amount, $vat_amount, $be, $password)
    {
        ...
        $vendor_id = !empty(Auth::guard('vendor')->user()->id) ? Auth::guard('vendor')->user()->id : $request['vendor_id'];
        ...
        if ($vendor) {
            $membership = Membership::create([
               ...
               // 'status' => $transaction_details != '"offline"' ? $request["order_status"] : 0,
               'status' => 1,
                ...
                'vendor_id' => $vendor->id,
                //'start_date' => now()->toDateTimeString('Y-m-d'),
                //'expire_date' => now()->toDateTimeString('Y-m-d'),
                'is_trial' => 0,
                'trial_days' => 0,
            ]);
        }
    ...
    public function onlineSuccess()
    {
        ...
        $information['ad_id'] = Car::where('order_id', $order_id)->first();
