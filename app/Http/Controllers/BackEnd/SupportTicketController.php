...
class SupportTicketController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    if (empty($role_id)) {
      $collection = SupportTicket::with('vendor')->when($status, function ($query, $status) {
        ...
        ->when($add_id, function ($query, $add_id) {
          return $query->where('ad_id', $add_id);
        })
        ->orderByDesc('id')
        ->paginate(10);
    } 
  ...
  //ticket_closed
  public function ticket_closed(Request $request, $id)
  {
    SupportTicket::where('id', $id)->update([
      'status' => 3,
      'blocked_by' => 'admin'
