...
class AdminController extends Controller
{
  ...
  public function redirectToDashboard()
  {
    ...
    $information['totalUser'] = Vendor::query()->count();
    ...
    $data = Car::select(DB::raw('MONTH(created_at) as month'), 'remove_option', DB::raw('COUNT(*) as count'))
        ->where('created_at', '>=', Carbon::create($currentYear)->startOfYear())
        ->whereIn('remove_option', ['I sold it on Listit', 'I sold it elsewhere'])
        ->groupBy('month', 'remove_option')
        ->get();
    ...
    $chartData = [
        ...
        'datasets' => [
            [
                'label' => 'I sold it on Listit',
                'data' => array_fill(0, 12, 0), // Initialize with 12 zeroes for each month
                'backgroundColor' => 'rgba(75, 192, 192, 0.2)',
                'borderColor' => 'rgba(75, 192, 192, 1)',
                'borderWidth' => 1,
            ],
            [
                'label' => 'I sold it elsewhere',
                'data' => array_fill(0, 12, 0), // Initialize with 12 zeroes for each month
